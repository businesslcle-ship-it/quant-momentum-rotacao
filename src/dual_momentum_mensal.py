...
print(
    f"Dual Momentum mensal (baseline) | {ret_estrategia.index[0]:%Y-%m}→{ret_estrategia.index[-1]:%Y-%m} | "
    f"Sharpe {sharpe:.2f} | MaxDD {drawdown_max:.0%} | ret {retorno_total:.0%} | "
    f"caixa {meses_caixa:.0%} | {trocas} trocas | hoje {posicao_hoje} | "
    f"B&H Sharpe {sharpe_bh:.2f} ret {retorno_bh:.0%} | CDI {retorno_cdi:.0%}"
)
